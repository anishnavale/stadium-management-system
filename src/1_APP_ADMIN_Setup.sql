...
execute immediate 'grant CREATE USER to '||user_name|| ' WITH ADMIN OPTION';
execute immediate 'grant ALTER USER to '||user_name|| ' WITH ADMIN OPTION';
dbms_output.put_line('Granted Roles and Privileges: CONNECT(WITH ADMIN OPTION), RESOURCE, CREATE VIEW, CREATE USER (WITH ADMIN OPTION), ALTER USER (WITH ADMIN OPTION) to '|| user_name);
